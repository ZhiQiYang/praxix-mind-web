...
const GoalCard = ({ goal }) => {
  ...
  // 計算已完成的子目標數量
  const completedSubgoals = subgoals?.filter(sg => sg.completed).length || 0;
  const totalSubgoals = subgoals?.length || 0;
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm hover:shadow transition-shadow duration-200">
      {/* 卡片頭部 */}
      <div className={`${bgColor} p-4 flex justify-between items-center rounded-t-lg`}>
        <h3 className="text-lg font-bold text-white">{title}</h3>
        <div className="text-white opacity-90 text-xl">
          {icon && <span>{icon}</span>}
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        {totalSubgoals > 0 && (
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-1">
              <span className="text-gray-600">子目標</span>
